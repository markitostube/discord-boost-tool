...
setTitle("IMarkitos | Server Booster")
...
async def boost_server(guildid, token):
  ...
  async with aiohttp.ClientSession(headers=headers) as ClientSession:
    async with ClientSession.get("https://discord.com/api/v9/users/@me/guilds/premium/subscription-slots") as nvmmm:
      if nvmmm.status == 200:
        ...
        for varr in idk_var:
          ...
          async with ClientSession.put(f"https://discord.com/api/v9/guilds/{guildid}/premium/subscriptions", json=payload) as boost_req:
            ...
            if "id" in btxt:
              print("Successfully Boosted Server")
            else:
